...
def download_password(password:str | int) :
    st.download_button(
        ...
        file_name="Recovery-Password.txt",
        mime="text/plain"
    )
...
def decrypt_previously_encrypted_image():
    ...
    st.warning("☢️ \nIf you encrypted the image using even a slightly different algorithm, the decrypted image will be corrupted.")

    img_size = None
    img_mode = None
    upload_types = ["enc"] if option != "Basic XOR Encryption" else ["jpg", "jpeg", "png", "gif"]
    ...
    if password and uploadedFile and option and buttonpressed:
        try:
            if option == "Basic XOR Encryption":
                encrypted_image = Image.open(uploadedFile)
            ...
            download_image(decrypted_image, "decrypted_image.png")
...
def main():
    ...
    if uploadedFile and option and password:
        ...
        with col1:
            ...
            download_image(original_image, "original_image.png")
        ...
        try:
            if option == options_tuple[0]:  # Basic XOR
                ...
                with col2:
                    ...
                    download_image(encrypted_image, "encrypted_image.png")
                decrypted_image = basic_xor.crypt(encrypted_image, int(password))
                with col3:
                    st.image(decrypted_image, caption="Decrypted Image", use_container_width=True)
                    download_image(decrypted_image, "decrypted_image.png")
            ...
            st.info("If you want to decrypt the image again, you can use the same password and algorithm used to encrypt the image.")
            st.warning("If you encrypted the image in PCBC mode, You get the height, width and color mode of the original image along with the password in the password download file.")
            st.html(
